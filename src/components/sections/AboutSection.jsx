    ...
    <div style={{ position: "fixed", inset: 0, background: DARK, overflowY: "auto", zIndex: 50, fontFamily: SANS }}>
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "2.5rem 2.5rem 5rem" }}>
        ...
        <div style={{ marginBottom: "3rem" }}>
          ...
          <p style={{ fontSize: 16, color: TEXT_PRIMARY, lineHeight: 1.85, margin: 0, fontWeight: 400, letterSpacing: "0.01em" }}>
        ...
        <div style={{ marginBottom: "3rem" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "1rem" }}>
            {[["U OF M", "Class of 2028"], ["MAJOR", "Electrical Eng."], ["LOCATION", "Ann Arbor, MI"]].map(([k, v]) => (
              <div key={k} style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 10, padding: "1.25rem", borderTop: `2px solid ${color}` }}>
                <div style={{ fontFamily: PF, fontSize: 6, color: color, letterSpacing: "0.12em", marginBottom: 10 }}>{k}</div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {HOBBIES.map(h => (
              <span key={h} style={{
                ...
                borderRadius: 20, color: TEXT_PRIMARY, fontWeight: 500, letterSpacing: "0.02em",
              }}>{h}</span>
            ))}
          </div>
        </div>
