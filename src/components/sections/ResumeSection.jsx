...
function ResumeBlock({ color, heading, items }) {
  return (
    <div style={{ marginBottom: "2.25rem" }}>
      <div style={{ fontFamily: PF, fontSize: 7, color: color, letterSpacing: "0.14em", marginBottom: "1.25rem", display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
        {items.map((item, i) => (
          <div key={i} style={{ borderLeft: `2px solid ${color}55`, paddingLeft: "1.25rem" }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: TEXT_PRIMARY, marginBottom: 3, letterSpacing: "0.01em" }}>{item.title}</div>
            <div style={{ fontSize: 13, color: color, marginBottom: item.detail ? 6 : 0, fontWeight: 600, letterSpacing: "0.02em" }}>{item.sub}</div>
            {item.detail && <div style={{ fontSize: 13, color: TEXT_MUTED, lineHeight: 1.75, letterSpacing: "0.01em" }}>{item.detail}</div>}
          </div>
        ))}
      </div>
    </div>
          ...
          <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 10, padding: "1.25rem 1.5rem", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            {[
              ...
              <div key={k} style={{ display: "grid", gridTemplateColumns: "100px 1fr", gap: "1rem", fontSize: 13 }}>
                <span style={{ fontWeight: 700, color: color, letterSpacing: "0.02em" }}>{k}</span>
