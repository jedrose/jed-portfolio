...
print(f"\\nAvg. Field Accuracy: {average_accuracy:.4f}")`,
  ...
  },
  {
    ...
    coverColor: "#059669",
    coverImage: "/engr100-imgs/final-design.png",
    ...
    images: [
      { src: "/engr100-imgs/schematic.png", contain: true },
      { src: "/engr100-imgs/prototypes.png", contain: true },
      { src: "/engr100-imgs/final-design.png", contain: true },
    ],
    sections: [
      {
        heading: "The Problem",
      ...
      },
      {
        ...
        body: "We worked through an objectives tree, a morphological chart, and a numerical evaluation matrix to compare approaches: infrared beams, camera-based vision, pressure mats, and ultrasonic ranging all made the list. We landed on a dual-sensor ultrasonic system that determines direction of travel based on which sensor is occluded first. The order-of-occlusion approach is robust: it does not need image processing, it works in any lighting condition, and the logic is straightforward to implement on an Arduino.",
        images: [1],
      },
      {
        heading: "Hardware and Wiring",
        body: "I was responsible for wiring the Arduino and all hardware integration. The device uses two ultrasonic sensors mounted on a 3D-printed bracket that attaches to the side of a door frame, using the opposite frame as a backstop. Each sensor pings independently. When someone walks through, the sensors fire in sequence and the order tells you which direction they are going. I wired both sensors to the Arduino's digital pins, handled the power routing, and ran hardware-level testing to confirm sensor responsiveness across different distances and angles.",
        images: [0],
      },
      {
        heading: "Programming and Logic",
        body: "The firmware is written in Arduino C++. The core logic: poll both sensors in a tight loop, track which one detects an object first, apply a timing window to debounce and reject false positives, then increment or decrement the running count. We also added physical buttons to manually adjust the count if needed, a 4-digit display showing the live total, and low-battery detection. An earlier prototype had the sensors angled downward from above the door frame, more precise for varying speeds, but the programming complexity made us pivot to the side-mount design which hit all functional requirements with a simpler implementation.",
      },
      {
        ...
        images: [2],
        videos: [
          { src: "/engr100-imgs/Mode Test.mov", label: "MODE TEST", type: "video/quicktime" },
          { src: "/engr100-imgs/Invert Test.mov", label: "INVERT TEST", type: "video/quicktime" },
        ],
      },
    ],
  },
